...
function renderTabla() {
  ...
  tbody.innerHTML = filtrats.map(d => `
    <tr class="${rowClass(d.gravedad)}">
      ...
      <td><button class="btn btn-outline btn-sm" onclick="obrirDetall(${d.id})">Ver Incedencia</button></td>
    </tr>`).join("");
}
  ...
  return new Promise(function (resolve) {
    ...
    window[cbName] = function (filas) {
      ...
      try {
        ...
        console.warn("[Vilamarina] Error procesando datos:", e.message);
      }
    ...
    s.onerror = function () {
      if (!terminado) { console.warn("[Vilamarina] No se pudo cargar desde Google Sheets."); resolve(); }
    };
    document.body.appendChild(s);
    setTimeout(function () { if (!terminado) { console.warn("[Vilamarina] Timeout cargando desde Google Sheets."); resolve(); } }, 10000);
  });
}
